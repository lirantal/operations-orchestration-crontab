...
function cliCheck() {
  ...
  var cli = commandLineArgs([{
    ...
    alias: 'u',
    type: String,
    description: 'Username for Operations Orchestration that is allowed to query the API'
  }, {
    name: 'password',
    alias: 'p',
    type: String,
    description: 'Password for the Username provided'
  }, {
    name: 'url',
    type: String,
    description: 'The URL where Operations Orchestration API is available. Example: http://localhost:8050'
  }, {
    name: 'remotehost',
    type: String,
    description: 'The remote host to connect to via SSH and execute the cron commands'
  }, {
    name: 'remoteport',
    type: String,
    description: 'The remote port to connect to via SSH'
  }, {
    name: 'remoteuser',
    type: String,
    description: 'The remote username to connect to via SSH'
  }, {
    name: 'remotepass',
    type: String,
    description: 'The remote password to connect to via SSH'
  }, {
    name: 'crontab',
    type: String,
    description: 'A local CRONTAB compatible filename to read and parse for creating OO scheduled flows'
  }, {
    name: 'log',
    type: String,
 ...
 * @return 	{object}	return a promise with array of all crontab entries read from the input file
 */
 ...
 * Recursive function to iterate an array of crontab entries and create a schedule flow
 *
 ...
 *
 * @method	createScheduledFlow
 * @param 	{object}		an array of crontab scheduling representation
 * @return 	{object}		returns a promise after all OO schedules have been made
 */
 ...
 *
 * @method	convertCrontabToCron
 * @param 	{object}		an array of quartz scheduling representation
 * @return 	{object}		returns a promise after converting all quartz syntax to crontab
 */
var convertCrontabToCron = function convertCrontabToCron(quartzResources) {

  return new Promise(function(resolve, reject) {

    var convertedCrons = [];
    quartzResources.forEach(function(crontabResource, crontabResourcesIndex, crontabResourcesArray) {
      ...
      if (crontab.length !== 2) {
        var errMsg = 'error parsing crontab entry';
        cliExitError(errMsg);
        return reject(errMsg);
      }

      convertedCrons.push(crontab);
      if (crontabResourcesIndex === (quartzResources.length - 1)) {
        return resolve(convertedCrons);
      }
...
var saveToLog = function saveToLog() {
  return new Promise(function(resolve, reject) {
    ...
    jsonfile.writeFile(options.log, log, {
      spaces: 2
    }, function(err) {
      if (err) {
        return reject(err);
